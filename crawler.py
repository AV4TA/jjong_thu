...
def fetch_kt_wiz_data():
    ...
    # 1. 📅 정규시즌 전 경기 일정 & 결과 수집
    # fromDate를 넉넉히 주되, 아래에서 시범경기 플래그와 날짜로 철저히 필터링
    sched_url = f"https://api-gw.sports.naver.com/schedule/games?upperCategoryId=kbaseball&fromDate={year}-03-01&toDate={year}-12-31&size=1500"
    ...
    if s_res and "result" in s_res:
        ...
        for g in games:
            ...
            category_name = str(g.get("categoryName", ""))
            league_type = str(g.get("leagueType", ""))
            title = str(g.get("title", ""))
            game_type = str(g.get("gameType", ""))

            # 🚫 [시범경기 4중 철통 차단]
            # 1. 카테고리나 타이틀에 '시범' 포함된 경우
            # 2. leagueType이 시범경기 코드인 경우
            # 3. 3월 23일 이전 경기 (KBO 시범경기 기간)
            is_exhibition = (
                "시범" in category_name or 
                "시범" in title or 
                "시범" in game_type or 
                league_type in ["DEMONSTRATION", "EXHIBITION"] or 
                g_date < f"{year}-03-23"
            )

            if is_exhibition:
                continue  # 시범경기는 절대 통계나 일정에 넣지 않고 스킵
    ...
    # 2. 📊 순위 정렬 및 게임차 직접 계산 (KBO 공식 룰: 승 / (승 + 패))
    def calc_wra(w, l):
        tot = w + l
    ...
    print(f"✅ [동기화 완료] 순위: {len(rankings)}팀 정규시즌 전용 산출 | 선발: {today_lineup['ktPitcher']} vs {today_lineup['oppPitcher']}")
